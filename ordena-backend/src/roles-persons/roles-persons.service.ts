...
export class RolesPersonsService {
  ...
  async create(createRolesPersonInput: CreateRolesPersonInput):Promise<RolesPerson> {
  const newRolesPerson = this.RolesPersonRepository.create(createRolesPersonInput)
  ...
  async findOne(id: number):Promise<RolesPerson>{
    ...
    if (!role_person) throw new NotFoundException('No hay un rol asignado a una persona con esa ID');
    return role_person;
  }
  ...
  async update(id: number, updateRolesPersonInput: UpdateRolesPersonInput) {
    ...
    const editedRolesPerson = Object.assign(role_person, updateRolesPersonInput);
