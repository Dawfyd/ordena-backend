...
export class BranchOfficesService {
  ...
  async create(createBranchOfficeInput: CreateBranchOfficeInput):Promise<BranchOffice> {
  const newBranchOffice = this.BranchOfficeRepository.create(createBranchOfficeInput)
  ...
  async findOne(id: number):Promise<BranchOffice>{
    ...
    if (!branch_office) throw new NotFoundException('No hay una sede con esa ID');
  ...
  async update(id: number, updateBranchOfficeInput: UpdateBranchOfficeInput) {
    ...
    const editedBranchOffice = Object.assign(branch_office, updateBranchOfficeInput);
