...
export class ProductsOrderedService {
  ...
  async create(createProductsOrderedInput: CreateProductsOrderedInput):Promise<ProductsOrdered> {
  const newProductsOrdered = this.ProductsOrderedRepository.create(createProductsOrderedInput)
  ...
  async findOne(id: number):Promise<ProductsOrdered>{
    ...
    if (!product_ordered) throw new NotFoundException('No hay un producto ordenado con esa ID');
    return product_ordered;
  }

  async update(id: number, updateProductsOrderedInput: UpdateProductsOrderedInput) {
    const product_ordered = await this.ProductsOrderedRepository.findOne(id);
    if (!product_ordered) throw new NotFoundException('No hay un producto ordenado con esa ID');
    
    const editedProductsOrdered = Object.assign(product_ordered, updateProductsOrderedInput);
